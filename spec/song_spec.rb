...
describe '#Song' do
  ...
  before(:each) do
    Album.clear()
    Song.clear()
    @album = Album.new({:name => "Giant Steps", :artist => "Ben", :genre => "folk", :year => "1990", :id => nil})
    @album.save()
  end
  #
  # describe('#==') do
  #   it("is the same song if it has the same attributes as another song") do
  #     song = Song.new({:name => "Naima", :album_id => @album.id, :lyrics => "", :writer => "", :id => nil})
  #     song2 = Song.new({:name =>"Naima", :album_id => @album.id, :lyrics => "", :writer => "",:id => nil})
  ...
  describe('.find_by_album') do
    it("finds songs for an album") do
      album2 = Album.new({:name => "Blue", :artist => "Ben", :genre => "folk", :year => "1990", :id => nil})
      album2.save
      song = Song.new({:name => "Naima", :album_id => @album.id, :lyrics => "", :writer => "",:id => nil})
      song.save()
      song2 = Song.new({:name =>"Naima", :album_id => @album.id, :lyrics => "", :writer => "",:id => nil})
